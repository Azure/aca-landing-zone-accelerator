...
param cosmosDbName string ='cosno-${workloadName}-${take(uniqueString(resourceGroup().id), 5)}-${environmentName}-${locationShortName}'
...
@description('Optional. The name of Cosmos DB resource. If set, it overrides the name generated by the template.')
param cosmosDbPrivateEndpointName string = 'pep-cosno-${workloadName}-${take(uniqueString(resourceGroup().id), 5)}-${environmentName}-${locationShortName}'

...
param privateLinkServiceName string = 'pl-afd-${workloadName}-${take(uniqueString(resourceGroup().id), 5)}-${environmentName}-${locationShortName}'
...
param frontDoorProfileName string = 'afd-${workloadName}-${take(uniqueString(resourceGroup().id), 5)}-${environmentName}-${locationShortName}'

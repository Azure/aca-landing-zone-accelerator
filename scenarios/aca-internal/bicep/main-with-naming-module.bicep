...
@minLength(2)
@maxLength(8)
@description('The name of the workload for the hub.')
param hubWorkloadName string

@minLength(2)
@maxLength(8)
@description('The name of the workload for the spoke.')
param spokeWorkloadName string
...
param hubResourceGroupName string = 'rg-${hubWorkloadName}-${environmentName}-${locationShortName}'
...
@maxLength(90)
@description('Optional. The name of the resource group to create the resources in. If set, it overrides the name generated by the template.')
param spokeResourceGroupName string = 'rg-${spokeWorkloadName}-${environmentName}-${locationShortName}'
...
module hubNaming 'modules/naming/naming.module.bicep' = {
...
module hub '01-hub/main.bicep' = {
  name: take('${deployment().name}-hub', 64)
  params: {
    location: location
    workloadName: hubWorkloadName
    environmentName: environmentName
...
module spokeNaming 'modules/naming/naming.module.bicep' = {
...
module spoke '02-spoke/main.bicep' = {
  name: take('${deployment().name}-spoke', 64)
  params: {
    ...
    environmentName: environmentName
    locationShortName: locationShortName
    spokeVNetName: spokeNaming.outputs.resourcesNames.spokeVNet
    tags: tags
...
module supportingServices '03-supporting-services/main.bicep' = {
  name: take('${deployment().name}-supportingServices', 64)
  scope: spokeResourceGroup
  params: {
    location: location
    workloadName: spokeWorkloadName
    environmentName: environmentName
    locationShortName: locationShortName
    containerRegistryName: spokeNaming.outputs.resourcesNames.containerRegistry
    containerRegistryUserAssignedIdentityName: spokeNaming.outputs.resourcesNames.containerRegistryUserAssignedIdentity
    containerRegistryPrivateEndpointName: spokeNaming.outputs.resourcesNames.containerRegistryPrivateEndpoint
    keyVaultName: spokeNaming.outputs.resourcesNames.keyVault
    keyVaultUserAssignedIdentityName: spokeNaming.outputs.resourcesNames.keyVaultUserAssignedIdentity
    keyVaultPrivateEndpointName: spokeNaming.outputs.resourcesNames.keyVaultPrivateEndpoint
    tags: tags
...
module containerAppsEnvironment '04-container-apps-environment/main.bicep' = {
  name: take('${deployment().name}-containerAppsEnvironment', 64)
  scope: spokeResourceGroup
  params: {
    location: location
    workloadName: spokeWorkloadName
...
module helloWorlSampleApp '05-hello-world-sample-app/main.bicep' = if (deployHelloWorldSample) {
  name: take('${deployment().name}-helloWorlSampleApp', 64)
  scope: spokeResourceGroup
  params: {
    location: location
    tags: tags
...
module applicationGateway '06-application-gateway/main.bicep' = if (deployHelloWorldSample) {
  name: take('${deployment().name}-applicationGateway', 64)
  scope: spokeResourceGroup
  params: {
    location: location
    workloadName: spokeWorkloadName
